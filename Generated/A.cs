...
file unsafe class ClassInformation : IComExposedClass
{
    ...
    static unsafe ComWrappers.ComInterfaceEntry* IComExposedClass.ComInterfaceEntries
    {
        get
        {
            if (_entries == null)
            {
                var entries = (ComWrappers.ComInterfaceEntry*)RuntimeHelpers.AllocateTypeAssociatedMemory(typeof(A), sizeof(ComWrappers.ComInterfaceEntry) * 1);
                var details = StrategyBasedComWrappers.DefaultIUnknownInterfaceDetailsStrategy.GetIUnknownDerivedDetails(typeof(IComInterface1).TypeHandle)!;
